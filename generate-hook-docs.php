...
namespace HivePress\Tools;

/**
 * Generate documentation for hooks in HivePress
 */
class HookDocsGenerator
{
     ...
     * Source path.
     */
    protected const SOURCE_PATH = 'hivepress/';
    ...
    protected static function getJSOutput(array $hook_list): string
    {
        ...
        foreach ($hook_list as $heading => $hooks) {
            foreach ($hooks as $hook => $details) {
                ...
                foreach ($details['files'] as $file) {
                    $summary .= ' located in ' . str_replace('hivepress/', '', $file['path']) . ': ' . $file['line'];

                    $output .= ',{';
                    $output .= 'fqsen: "' . $name . '",';
                    $output .= 'name: "' . $hook . '",';
                    $output .= 'summary: "' . $summary . '",';
                    $output .= 'url: "' . str_replace('../', 'https://hivepress.github.io/code-reference/', self::getFileURL($file)) . '"';
                    $output .= '}';
                }
            }
        }
